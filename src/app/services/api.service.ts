...
import { ICategory, IGuest, IOrder, IPosterOrder, IProduct, Menu } from '../models/models';
...
export class ApiService {
  ...
  public getGuest(phone: string): Promise<IGuest | undefined | null> {
    ...
    return this.http
      ...
      .toPromise().then((guest) => ({
        id: guest!['client_id'],
        firstname: [guest!['firstname'], guest!['lastname']].join(' '),
        discount: Number(guest!['client_groups_discount'] || guest!['discount_per'] || '0'),
      }))
  ...
  public createGuest(phone: string, client_name: string): Promise<{client_id: string}> {
    ...
    return this.http
      ...
      .toPromise()
      .catch(this.getErrorHandler({ value: '', message: 'Fail to create account', btnText: "Ok, I'll try later" }));
  }
